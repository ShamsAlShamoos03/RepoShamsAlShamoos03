...
EXEC sp_CreateDailyActivityEvidence01 
 ...
 Job_sp_CreateDailyActivityEvidence01
	*/
...
BEGIN
    ...
    BEGIN
		...
		DECLARE @DateCopyAmar01 NVARCHAR(10) = dbo.MiladiToShamsi(dbo.AddDaysToDate(dbo.ShamsiToGregorian(@NewDate), -1));

	EXEC	sp_CreateDailyPlanEvidence01 
    @DailyActivityEvidence01ID = @NewDailyActivityID,
  @DateAmar = @NewDate,
  @DateCopyAmar01 =  @DateCopyAmar01,
   @isnullDailyPlan = @isnullDailyPlan;


    EXEC [dbo].[CreateUnitCountAmar01_Tbl] @DailyActivityEvidence01ID = @NewDailyActivityID  

    EXEC [dbo].[Update_PersonalCorrespondence01_DocumentNO01]  NULL;
