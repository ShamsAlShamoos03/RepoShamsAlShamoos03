   ...
   SELECT    [DailyPlanEvidenceStatAmar01Summary01ID]
      ...
      ,[DailyPlanEvidenceStatAmar01Summary01_Tbl].[MelliCode]
      ,[StatAmar01]	 ,     CatStatAmar01.Title   AS strStatAmar01

	   , dbo.GetPersianYear([StartDate]) AS PersianYear -- خروجی: 1403
	   -- RewardDaysCount
	   ,ISNULL(RewardDaysCount, 0) AS RewardDaysCount
	   ,ISNULL(TotalVacationDaysCount, 0) AS TotalVacationDaysCount
	  
      ,[StartDate] as [StartDate01]
      ...
      ,[Sortable01Id]
        ...
        WHEN [StatAmar01] = 581 THEN 
            SUM(CAST(CONVERT(INT, COALESCE(DurationDays, 0) - COALESCE(HolidayDaysCount, 0)) AS DECIMAL(18,2))) OVER(PARTITION BY [DailyPlanEvidenceStatAmar01Summary01_Tbl].[MelliCode], [StatAmar01],dbo.GetPersianYear([StartDate]))
        WHEN [StatAmar01] != 581 THEN 
            SUM(CAST([DailyPlanEvidenceStatAmar01Summary01_Tbl].[DurationDays] AS DECIMAL(18,0))) OVER(PARTITION BY [DailyPlanEvidenceStatAmar01Summary01_Tbl].[MelliCode], [StatAmar01],dbo.GetPersianYear([StartDate]))
        ELSE 0 
    END AS CombinedTotal	   
	
	  	  , CASE 
        WHEN [StatAmar01] = 581 THEN 
            SUM(CAST(CONVERT(INT, COALESCE(DurationDays, 0) - COALESCE(HolidayDaysCount, 0)) AS DECIMAL(18,2))) OVER(PARTITION BY [DailyPlanEvidenceStatAmar01Summary01_Tbl].[MelliCode], [StatAmar01])
        WHEN [StatAmar01] != 581 THEN 
            SUM(CAST([DailyPlanEvidenceStatAmar01Summary01_Tbl].[DurationDays] AS DECIMAL(18,0))) OVER(PARTITION BY [DailyPlanEvidenceStatAmar01Summary01_Tbl].[MelliCode], [StatAmar01])
        ELSE 0 
    END AS CombinedTotal2	   
	


	,  SUM(CONVERT(INT, COALESCE(DurationDays, 0) - COALESCE(HolidayDaysCount, 0))) OVER(PARTITION BY [DailyPlanEvidenceStatAmar01Summary01_Tbl].[MelliCode]) AS TotalSumStatAmar01VNoHolidayPerMelliCode

      ,SUM(CONVERT(INT, COALESCE(DurationDays, 0) - COALESCE(HolidayDaysCount, 0))) OVER(PARTITION BY [DailyPlanEvidenceStatAmar01Summary01_Tbl].[MelliCode], [StatAmar01]) AS TotalSumStatAmar01333VNoHolidayPerMelliCodeAndStatAmar
...
--WHERE EMP_DAT IS NOT NULL;
  ...
  FROM [dbo].[DailyPlanEvidenceStatAmar01Summary01_Tbl]
  ...
  --order by [DailyPlanEvidence01ID],StartDate
    --order by [MaxContinuityHolidayDays]
	 --order by StartDate
	-- where  [DailyPlanEvidenceStatAmar01Summary01_Tbl].[DailyPlanEvidence01ID]  is not NULL
	--and [StatAmar01] =581
	  --and [DailyPlanEvidenceStatAmar01Summary01_Tbl].[MelliCode] ='5740003369'   
	  
	  	  --and [DailyPlanEvidenceStatAmar01Summary01_Tbl].[MelliCode] ='5749561380'    
