CREATE OR ALTER VIEW dbo.Report2View02
AS
WITH ImagesCTE AS
(
    SELECT 
...
WHERE img1.IndexImage IS NOT NULL
  AND NewsDateSham > '1403/09/17'
  AND cs.CategrySubId = 5

-- ORDER BY داخل VIEW با TOP مجاز است، برای مرتب‌سازی بهتر هنگام SELECT از VIEW انجام بده
;
